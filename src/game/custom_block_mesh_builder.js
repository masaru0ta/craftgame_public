...
class CustomBlockMeshBuilder {
  ...
  /**
   * 複数のカスタムブロックを1つのメッシュに統合
   * @param {Array} blocks - ブロック配列 [{ voxelData, materials, position: {x,y,z} }, ...]
   * @param {number} voxelSize - ボクセルサイズ
   * @returns {THREE.Mesh|null} 統合されたメッシュ（ブロックがない場合はnull）
   */
  buildIntegratedMesh(blocks, voxelSize = 0.125) {
    ...
    // 全マテリアルを収集してインデックス化
    const materialMap = new Map(); // base64 or color -> index
    const allMaterials = [];
    const matIndices = []; // マテリアルごとのインデックス配列
    ...
    for (const block of blocks) {
      const { voxelData, materials, position } = block;
      const blockOffsetX = position.x;
      const blockOffsetY = position.y;
      const blockOffsetZ = position.z;

      // このブロックのマテリアルをグローバルインデックスにマッピング
      const localToGlobalMat = [];
      for (let i = 0; i < materials.length; i++) {
        const mat = materials[i];
        // マテリアルのキーを生成（テクスチャまたは色で識別）
        const matKey = mat.map ? (mat.map.image ? mat.map.image.src : `mat_${allMaterials.length}`) : `color_${mat.color ? mat.color.getHex() : 0x808080}`;

        if (!materialMap.has(matKey)) {
          const globalIdx = allMaterials.length;
          materialMap.set(matKey, globalIdx);
          allMaterials.push(mat);
          matIndices.push([]);
        }
        localToGlobalMat[i] = materialMap.get(matKey);
      }

      // 各面方向ごとにグリーディメッシング（buildWithUVGreedyと同じロジック）
      for (const config of faceConfigs) {
        const { axis, u, v, offset, brightness, name } = config;
        ...
        for (let d = 0; d < gs; d++) {
          ...
          for (let vPos = 0; vPos < gs; vPos++) {
            for (let uPos = 0; uPos < gs; uPos++) {
              ...
              const localMatIdx = matValue - 1;
              const globalMatIdx = localToGlobalMat[localMatIdx] !== undefined ? localToGlobalMat[localMatIdx] : 0;
              ...
              // 頂点を追加（ブロック位置オフセットを含む）
              for (const vert of verts) {
                positions.push(
                  vert[0] * vs - off + blockOffsetX,
                  vert[1] * vs - off + blockOffsetY,
                  vert[2] * vs - off + blockOffsetZ
                );
                colors.push(brightness, brightness, brightness);
              }
    ...
    const materialGroups = [];
    let indexOffset = 0;
    for (let i = 0; i < matIndices.length; i++) {
      if (matIndices[i].length > 0) {
        materialGroups.push({
          start: indexOffset,
          count: matIndices[i].length,
          materialIndex: i
        });
        indices.push(...matIndices[i]);
        indexOffset += matIndices[i].length;
      }
    }
    ...
    const mesh = new this.THREE.Mesh(geometry, allMaterials);
    mesh.userData.isCustomBlockMesh = true;
    return mesh;
  }
