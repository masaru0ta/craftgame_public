...
class TextureLoader {
    ...
    constructor() {
        ...
        this.structures = [];    // 構造物リスト
        this.recipes = [];       // レシピ定義リスト
        ...
        this._texNameToAtlasUV = new Map(); // テクスチャ名 -> { offsetX, offsetY, scaleX, scaleY }
    ...
    _createAtlas() {
        ...
        for (const block of this.blocks) {
            ...
            // カスタムブロックのmaterial_1/2/3テクスチャも収集
            if (block.shape_type === 'custom') {
                for (const matKey of ['material_1', 'material_2', 'material_3']) {
                    const texName = block[matKey];
                    if (texName && this._textureCache.has(texName)) {
                        usedTextures.add(texName);
                    }
                }
        ...
        // テクスチャ名→アトラスUVマップを構築
        for (const [texName, { gridX, gridY }] of texNameToIndex) {
            this._texNameToAtlasUV.set(texName, {
                offsetX: gridX * uvScale,
                offsetY: 1 - (gridY + 1) * uvScale,
                scaleX: uvScale,
                scaleY: uvScale
            });
        }
    ...
    _createAtlasShaderMaterial(atlasTexture, gridSize) {
        const vertexShader = `
            ...
            attribute float lightLevel; // ライトファクター（0.1〜1.0）
            attribute float aoLevel;    // AO係数（0.35〜1.0）
            varying vec2 vUv;
            ...
            void main() {
                ...
                vAoFactor = aoLevel;
                vNormal = normal; // ワールド空間（チャンクは回転なし）
        ...
        const fragmentShader = `
            ...
            void main() {
                ...
                // ライトマップの明るさとAOを適用
                vec3 finalColor = texColor.rgb * irradiance * vLightLevel * vAoFactor;
    ...
    getWaterMaterial() {
        ...
        if (!this._atlasTexture) return null;
        ...
        const fragmentShader = `
            ...
            void main() {
                ...
                vec3 finalColor = waterColor * irradiance * vLightLevel * vAoFactor;
        ...
        this._waterMaterial = new THREE.ShaderMaterial({
            uniforms: {
                atlasTexture: { value: this._atlasTexture },
                ambientLightColor: { value: new THREE.Color(0.4, 0.4, 0.4) },
                directionalLightColor: { value: new THREE.Color(1.0, 1.0, 1.0) },
                directionalLightDirection: { value: new THREE.Vector3(0.5, 1.0, 0.3).normalize() }
            },
    ...
    /**
     * テクスチャ名からアトラスUV情報を取得
     * @param {string} texName - テクスチャファイル名
     * @returns {{ offsetX: number, offsetY: number, scaleX: number, scaleY: number }}
     */
    getAtlasUVByTexName(texName) {
        return this._texNameToAtlasUV.get(texName) || {
            offsetX: 0,
            offsetY: 0,
            scaleX: 1 / (this._atlasSize || 1),
            scaleY: 1 / (this._atlasSize || 1)
