...
class LoDTestApp {
    ...
    async init() {
        ...
        // UI初期化（setLoD0RangeがlastChunkXをリセットするため、updateViewPositionの前に実行）
        this._initUI();
    ...
    _handleManualMovement() {
        ...
        // 高度に応じて速度を変える（カメラY / 100、最低1.0倍）
        const altitudeFactor = Math.max(1.0, 1.0 + this.camera.position.y / 100);
        const moveAmount = this.moveSpeed * altitudeFactor * delta;
