...
- **テストコードは実装コードに依存しない**（下記「実装非依存の原則」参照）
- 実装前にテストを実行し、失敗することを確認

#### 2.2 実装非依存の原則

**ルール: コード実装に依存するテストコードを書いてはいけない。**

テストに必要な情報は：
1. **仕様書に定義する** - 公開メソッド、CSSセレクタ、イベント等を仕様書に追記
2. **実装に依存しないテストを書く** - ユーザーから見える振る舞いのみを検証

テストコードは仕様書に定義された公開インターフェースのみを使用する。

**許可される参照:**
- 仕様書で定義された公開メソッド（例: `getScene()`, `getCamera()`）
- 仕様書で定義されたCSSセレクタ（例: `.editor-container`, `data-slot`）
- 仕様書で定義されたコールバック、イベント

**禁止される参照:**
- 内部プロパティ（例: `editor.scene`, `editor.horizontalAngle`）
- 仕様書に記載のない内部メソッド
- 実装の詳細に依存するセレクタ

**必要な情報がテストできない場合の対応:**
1. **仕様書に追加**: テストに必要なCSSセレクタや公開メソッドを仕様書に追記
2. **間接的なテスト**: 公開メソッドの戻り値や状態変化で検証（例: カメラ位置から角度を計算）
3. **振る舞いのテスト**: 内部状態ではなく、ユーザーから見える結果をテスト
...
| 内部プロパティを参照する      | 公開メソッドを使用、必要なら仕様書に追加 |
...
- **コード実装に依存するテストを書かない**（必要な情報は仕様書に定義する）
- テストが全件パスするまでコミットしない
- 実装前に必ず仕様書を確認する
