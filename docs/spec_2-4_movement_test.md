...
| STEP_CHECK_DISTANCE | 0.3 | 進行方向の高さチェック距離（ブロック） |
...
3. **進行方向0.3ブロック前の位置**での障害物高さをチェック
4. その高さがプレイヤーの足元から0.5ブロック以内
5. ブロックの上に十分なスペースがある（プレイヤーの高さ分）
6. 瞬時にY座標をブロック上面に移動（ジャンプ速度は付与しない）

**カスタムブロック対応:**
- カスタムブロックの当たり判定ボクセル（4x4x4）の上面にもステップアップ可能
- 例: 下半分（y=0,1）のみ当たり判定があるハーフブロック（高さ0.5）
- 例: 階段を前から登る → 進行方向前方は低い部分（0.25〜0.5）→ ステップアップ

#### オートジャンプ機能

中程度の段差（0.5超〜1.0ブロック）に衝突すると、自動でジャンプする機能。
...
2. 移動先にブロックがあり、進めない（水平衝突）
3. **進行方向0.3ブロック前の位置**での障害物高さをチェック
4. その高さが0.5ブロック超かつ1.0ブロック以内
5. ブロックの上に十分なスペースがある
6. ジャンプ速度（JUMP_VELOCITY）を付与してジャンプ

**階段での動作例:**
- 前から登る: 進行方向0.3ブロック前は低い部分（0.25〜0.5）→ ステップアップ
- 後ろから登る: 進行方向0.3ブロック前は高い部分（1.0）→ オートジャンプ
