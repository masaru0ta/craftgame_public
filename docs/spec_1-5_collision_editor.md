...
- 当たり判定編集モード時:
  ...
  - 床面のグリッド線は 4x4
  - 下部のコントロール枠には左端に「自動作成」ボタン、右端に「衝突テスト」ボタンを配置
- 見た目編集モード時:
...
「自動作成」ボタンをクリックすると、見た目のボクセルデータを元に当たり判定を自動生成する。
...
| 1本指ドラッグ | 視点回転 |
| タップ（短くタッチ） | ボクセル配置 |
| ロングプレス（500ms長押し） | ボクセル削除 |
| 2本指ピンチ | ズーム |

- ロングプレスで削除する際、対応デバイスでは振動フィードバックがある
...
- 物理演算は60fps固定タイムステップで実行（ディスプレイのリフレッシュレートに依存しない）
...
| 自動作成ボタン | `.auto-create-btn` | 当たり判定モード時に左端に表示 |
| 衝突テストボタン | `.check-btn` | 当たり判定モード時に右端に表示 |
