...
| 定数 | 値 | 説明 |
|------|-----|------|
| BALL_COUNT | 30 | 同時存在ボール数 |
| BALL_DIAMETER | 0.1 | ボール直径（ブロック単位） |
| FIXED_TIMESTEP | 1/60 | 固定タイムステップ（60fps） |
| GRAVITY | -9.8 * 0.2 | 重力加速度（約-1.96） |
| RESTITUTION | 0.7 | 反発係数（70%反発） |
| VOXEL_SIZE | 0.25 | ボクセルサイズ（4x4x4グリッド） |
| FALL_THRESHOLD | -2 | 奈落判定のY座標閾値 |
| BALL_COLLISION | true | ボール同士の衝突を有効化 |
...
- 新しい球体は常にブロック中心から重力の反対方向、ブロックサイズ2倍の距離のポイントに生成される
- 同時に30個の球体が存在
- 球体が奈落（y座標が一定値以下）に落ちたら、初期位置に再生成される（継続的に落下）
- **当たり判定ボクセルとのみ衝突・反射する**（床面や壁面との衝突は行わない）
...
| 衝突テストボタン（アクティブ時） | `.check-btn.active` | 衝突テスト実行中 |
| コントロールパネル（カスタムブロック用） | `.control-panel.has-check-btn` | 衝突テストボタンがある場合のレイアウト |
| ミニプレビュー（canvas） | `#modeToggle canvas` | モード切替ボタン内のミニプレビュー表示 |
...
| ミニプレビュー | loadBlock直後にミニプレビューが更新される、モード切替でミニプレビューが切り替わる |
| エディタ切り替え | 標準ブロック→カスタムブロック切り替え後に衝突テストが正常に動作する |
