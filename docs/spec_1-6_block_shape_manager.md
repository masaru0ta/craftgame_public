...
| ブラシサイズボタン | `.brush-size-btn` | 1-4と同じサイズ `clamp(24px, 5vw, 48px)` |
| 衝突テストボタン | `.check-btn` | 緑背景、スロット画像と同じ高さ |
| 自動作成ボタン | `.auto-create-btn` | 青背景、スロット画像と同じ高さ |
...
- 処理ロジックの要件１つずつ動作検証
- テクスチャ編集テスト:
  - 代表色を変更して保存後、ファイル名が維持される
  - 代表色を変更して保存後、テクスチャ画像（image_base64）が維持される
- BlockEditorUI統合テスト:
