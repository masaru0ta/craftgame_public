...
- 左クリックで選択したボクセルを削除
- ブラシサイズ:
  - [1] 1x1x1 ボクセル（単一）の設置・削除
  - [2] 2x2x2 ボクセル、グリッド座標(0,2,4,6)にスナップ
  - [4] 4x4x4 ボクセル、グリッド座標(0,4)にスナップ
...
- [ ] マテリアルスロットクリックでテクスチャ選択モーダルが表示される
- [ ] `setMaterial()` でマテリアルが反映される
- [ ] `setCurrentMaterial()` でマテリアル選択が変更される
- [ ] BGボタンクリックで背景色が切り替わる（黒→青→緑→黒）

### テクスチャ選択モーダル

- [ ] マテリアルスロットクリックでモーダルが表示される
- [ ] モーダルにテクスチャ一覧が表示される
- [ ] 「なし」が先頭に表示される
- [ ] 「追加」が最後尾に表示される
...
- [ ] `getCurrentMaterial()` で現在選択中のマテリアル番号を取得できる
- [ ] `setBrushSize(size)` でブラシサイズが変更される
- [ ] `getBrushSize()` で現在のブラシサイズを取得できる
- [ ] `setEditMode(mode)` で編集モードが変更される
- [ ] `getEditMode()` で現在の編集モードを取得できる
- [ ] `getVoxelLookData()` でBase64エンコードされたデータが取得できる
- [ ] `setBackgroundColor(color)` で背景色が変更される
- [ ] `toggleBackgroundColor()` で背景色を切り替えられる
...
- [ ] 初期表示でFRONTが正面になっている（horizontalAngle=0）
- [ ] 初期表示で垂直角度20度になっている（verticalAngle=20）
- [ ] 初期表示でカメラ距離3になっている（cameraDistance=3）
- [ ] 8x8x8のボクセルグリッド表示の実装コードがある
- [ ] 床面グリッド線（8x8）表示の実装コードがある
- [ ] FRONT, RIGHT, LEFT, BACKテキスト表示の実装コードがある
- [ ] ボクセルへのマテリアルテクスチャ適用の実装コードがある

### カメラ操作

- [ ] マウス左クリックのドラッグで視点を回転できる
- [ ] 上下の傾きが上側90度、下側90度までに制限される
- [ ] 右にドラッグするとブロックが右に回転する。（右に100pxドラッグして horizontalAngle が減少する）
- [ ] マウスホイールで拡大縮小できる
...
- [ ] 右クリックでボクセルを配置できる（getVoxelLookData()でデータ変化を確認）
- [ ] 左クリックでボクセルを削除できる（getVoxelLookData()でデータ変化を確認）
- [ ] 8x8x8の範囲外にはボクセルを配置できない
- [ ] ブラシサイズ1で1x1x1ボクセル（単一）の設置・削除
- [ ] ブラシサイズ2で2x2x2ボクセル、グリッド座標(0,2,4,6)にスナップ
- [ ] ブラシサイズ4で4x4x4ボクセル、グリッド座標(0,4)にスナップ
- [ ] カーソル位置のボクセルハイライト表示（緑面、赤辺）の実装コードがある
- [ ] ボクセルが無い場合の床面グリッドハイライトの実装コードがある
