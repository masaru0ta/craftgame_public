...
- 以下は 1-3 と異なる点
  ...
  - [1x] を選択すると 1x1x1 ボクセル（単一）の設置・削除
  - [2x] を選択すると 2x2x2 ボクセル（立方体）の設置・削除。ハイライトする面の大きさも 2x2 になる。ブロック全体を 4x4x4 に区切ったグリッド（座標0,2,4,6）でのみ設置・削除ができる。
  - [4x] を選択すると 4x4x4 ボクセル（立方体）の設置・削除。ハイライトする面の大きさも 4x4 になる。ブロック全体を 2x2x2 に区切ったグリッド（座標0,4）でのみ設置・削除ができる。
...
- [ ] [1x]選択時は1x1x1ボクセル（単一）の設置・削除
- [ ] [2x]選択時は2x2x2ボクセル（立方体）の設置・削除、ハイライトも2x2、グリッド座標(0,2,4,6)にスナップ
- [ ] [4x]選択時は4x4x4ボクセル（立方体）の設置・削除、ハイライトも4x4、グリッド座標(0,4)にスナップ
