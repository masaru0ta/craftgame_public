...
- モード切替ボタン内にはテキストではなく、ミニプレビュー（canvas）を表示
  - 見た目編集モードでは当たり判定ボクセルを表示
  - 当たり判定編集モードでは見た目ボクセルを表示
  - ボクセルの向きは中央の3Dプレビューのカメラ角度と常に連動
...
| 「追加」    | `onTextureAdd`コールバックで外部に通知（アップロード処理は外部で実装）。テストページではファイル選択ダイアログを表示し、選択された画像をGAS APIでアップロードする |
...
- グリッドの外側に FRONT, RIGHT, LEFT, BACK をテキスト表示
  - フォント: 20px Arial、色: #ffffff（1-3 StandardBlockEditorと同一仕様）
  - ラベル位置: 床面の高さ（Y=-0.5）で各方向に距離1の位置
    - FRONT: [0, -0.5, 1]
    - BACK: [0, -0.5, -1]
    - LEFT: [-1, -0.5, 0]
    - RIGHT: [1, -0.5, 0]
...
モバイル端末でのタッチ操作をサポートする。

| 操作 | 動作 |
|------|------|
| 1本指ドラッグ | カメラ回転（マウスドラッグと同等） |
| ピンチイン/アウト | ズーム（ホイールスクロールと同等） |

**実装詳細:**
- `touchstart`, `touchmove`, `touchend`, `touchcancel` イベントをリッスン
- 1本指タッチ: マウスドラッグと同じカメラ回転処理
- 2本指タッチ: 2点間の距離変化でズーム（ピンチ操作）
- タッチイベント処理時は `event.preventDefault()` でスクロール等を防止
- ドラッグ判定のしきい値（5ピクセル）はマウスと共通
...
右クリックを長押しすることで、同じ方向に連続でボクセルを設置できる。

| 項目 | 仕様 |
|------|------|
| トリガー | 右クリック押下開始 |
| 最初の設置 | 即座に1個目を設置 |
| 連続設置間隔 | 0.3秒（300ms）ごとに1個ずつ追加 |
| 設置方向 | 最初に設置した面の法線方向に固定 |
| 終了条件 | 右クリックを離す、または範囲外に到達 |
| マウス移動 | 連続設置中のマウス移動は無視（最初の位置から延長し続ける） |

**状態管理:**
- `getContinuousPlacementState()`: 連続設置状態を取得（テスト用）
  - `active`: boolean - 連続設置中かどうか
  - `direction`: {x, y, z} | null - 設置方向（法線ベクトル）
  - `lastCoord`: {x, y, z} | null - 最後に設置した座標
- ボクセル削除時にパーティクルエフェクトを表示
  - パーティクル数: 1個（削除ボクセルあたり）
  - パーティクルサイズ: 0.16（ボクセル単位）
  - パーティクル寿命: 0.8秒
  - パーティクル重力加速度: 4.0
  - パーティクルの色: 削除したボクセルのマテリアルに対応したテクスチャの代表色（白フォールバック）
  - パーティクルは放射状に飛散し、重力で落下
  - フェードアウトしながら消滅
  - `getParticleSystem()` でテスト用にParticleSystemインスタンスを取得可能
- ブラシサイズ:
...
- テクスチャと頂点カラーが乗算される
- `CustomBlockMeshBuilder.FACE_BRIGHTNESS` で明るさ定数を取得可能
...
| モード切替ボタン | `.mode-toggle-btn` | look/collision切替、canvas要素を含む |
| ミニプレビューcanvas | `.mode-toggle-btn canvas` | ミニプレビュー表示用 |
...
| 3Dプレビュー表示 | 8x8x8ボクセルグリッド、カメラ初期位置、床面グリッド線、方向ラベル（フォント20px白色） |
| モード切替ミニプレビュー | canvas表示、見た目/当たり判定の逆データ表示、カメラ角度連動 |
| カメラ操作 | マウスドラッグ回転、角度制限、ホイールズーム |
| ボクセル編集 | 右クリック配置、左クリック削除、ブラシサイズ別動作、範囲制限、ハイライト表示 |
| 連続設置機能 | 右クリック長押しで0.5秒ごとに連続設置、方向固定、マウス移動無視、状態取得 |
